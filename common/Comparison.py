...
class Comparison:
    ...
    def find_matching(self):
        ...
        for i in range(len(self.collection1.products_timestamps)):
            ...
            matching_products.append([timestamps1[i], self.collection1.products_timestamps[i][0], self.collection2.products_timestamps[pid][0]])
            self.logger.info("Found matching for %s between %s and %s" %
                      (timestamps1[i], self.collection1.products_timestamps[i][0], self.collection2.products_timestamps[pid][0]))
    ...
    def one_by_one(self):
        ...
        for match in self.matching_products:
            ...
            for band in self.bands_definition_acix:
                ...
                self.logger.info("One-by-one comparison RMSE, %s, %s, %s, %8.4f" % (match[0], band[1], band[0], rmse))
